...
public class GoReference extends GoReferenceBase<GoReferenceExpressionBase> {
  ...
  static PsiFile getContextFile(@NotNull ResolveState state) {
    PsiElement element = GoPsiImplUtil.getContextElement(state);
  ...
  public boolean processResolveVariants(@NotNull GoScopeProcessor processor) {
    ...
    ResolveState state = GoPsiImplUtil.createContextOnElement(myElement);
  ...
  private boolean processQualifierExpression(@NotNull GoFile file,
    ...
    if (target instanceof GoTypeOwner) {
      GoType type = GoPsiImplUtil.typeOrParameterType((GoTypeOwner)target, GoPsiImplUtil.createContextOnElement(myElement));
      if (type instanceof GoCType) return processor.execute(myElement, state);
      if (type != null) {
        if (!processGoType(type, processor, state)) return false;
        GoTypeReferenceExpression ref = GoPsiImplUtil.getTypeRefExpression(type);
  ...
  private boolean processExistingType(@NotNull GoType type, @NotNull GoScopeProcessor processor, @NotNull ResolveState state) {
    ...
    if (!(myFile instanceof GoFile) || !GoPsiImplUtil.allowed(file, myFile, ModuleUtilCore.findModuleForPsiElement(myFile))) return true;

    boolean localResolve = isLocalResolve(myFile, file);

    GoTypeSpec parent = GoPsiImplUtil.getTypeSpecSafe(type);
    boolean canProcessMethods = state.get(DONT_PROCESS_METHODS) == null;
    if (canProcessMethods && parent != null && !GoPsiImplUtil.processNamedElements(processor, state, parent.getMethods(), localResolve, true)) return false;
    ...
    if (type instanceof GoStructType) {
      ...
      for (GoFieldDeclaration d : ((GoStructType)type).getFieldDeclarationList()) {
        if (!GoPsiImplUtil.processNamedElements(processor, state, d.getFieldDefinitionList(), localResolve)) return false;
        ...
        if (ref != null) {
          ...
          if (!GoPsiImplUtil.processNamedElements(processor, state, ContainerUtil.createMaybeSingletonList(anon), localResolve)) return false;
        }
      }
      ...
      if (!GoPsiImplUtil.processNamedElements(processor, state, ((GoInterfaceType)type).getMethods(), localResolve, true)) return false;
  ...
  private boolean processReceiver(@NotNull GoScopeProcessor processor, @NotNull ResolveState state, boolean localResolve) {
    ...
    return GoPsiImplUtil.processNamedElements(processor, state, delegate.getVariants(), localResolve);
  }
  ...
  private boolean processSelector(@NotNull GoSelectorExpr parent,
    ...
    if (list.size() > 1 && list.get(1).isEquivalentTo(another)) {
      ...
      GoType type = e.getGoType(GoPsiImplUtil.createContextOnElement(myElement));
  ...
  protected boolean processFileEntities(@NotNull GoFile file,
    ...
    if (!GoPsiImplUtil.processNamedElements(processor, state, file.getConstants(), o -> !Comparing.equal(GoConstants.IOTA, o.getName()) ||
                                                                    !GoPsiImplUtil.builtin(o) ||
           PsiTreeUtil.getParentOfType(GoPsiImplUtil.getContextElement(state), GoConstSpec.class) != null, localProcessing, false)) return false;
    if (!GoPsiImplUtil.processNamedElements(processor, state, file.getVars(), localProcessing)) return false;
    Condition<GoNamedElement> dontProcessInit = o -> o instanceof GoFunctionDeclaration && !Comparing.equal(o.getName(), GoConstants.INIT);
    if (!GoPsiImplUtil.processNamedElements(processor, state, file.getFunctions(), dontProcessInit, localProcessing, false)) return false;
    return GoPsiImplUtil.processNamedElements(processor, state, file.getTypes(), localProcessing);
  }
